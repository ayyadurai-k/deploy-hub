...
# -------- user_service.resolve_google (login only) --------
...
def test_resolve_google_creates_user_on_first_login():
    ...
    result = user_service.resolve_google(identity, tokens)
...
# -------- user_service.resolve_github (login only) --------

@pytest.mark.django_db
def test_resolve_github_creates_user_with_email():
    identity = GitHubIdentity(user_id=99, login="alice", name="Alice", email="alice@gh.com", avatar_url="")
    tokens = GitHubTokens(access_token="a")
    result = user_service.resolve_github(identity, tokens)
    assert result.user_created and result.profile_created
    assert GitHubProfile.objects.filter(github_user_id=99).exists()
...
def test_resolve_github_login_without_email_raises():
    ...
    with pytest.raises(OAuthError):
        user_service.resolve_github(identity, tokens)
