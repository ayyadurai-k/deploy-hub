...
- **Linking is IN SCOPE (re-enabled 2026-05-17).** When a user is already signed in and clicks "Connect <other provider>" on the dashboard, the SPA hits `POST /api/v1/oauth/<provider>/link-start` with the existing Bearer token. The backend issues a state envelope with `intent="link"` and `owner_user_id=request.user.id`, sets the state cookie, and returns the provider authorize URL in JSON; the SPA then does a `window.location` navigation to complete the OAuth round-trip. The callback recognises `intent="link"` and routes to `link_google` / `link_github` in `user_service`, which **attach the new profile to the existing user** rather than creating a fresh one. No new JWT is minted — the user's session continues.
- **Collision handling (locked):** at link time, three branches in `link_*`:
  1. No existing profile for this `(provider, provider_user_id)` → attach to owner. (Triggers a synchronous repo sync for GitHub, matching the login-flow first-sync behaviour.)
  2. Existing profile already attached to the same `owner_user_id` → refresh stored token, no-op-ish.
  3. Existing profile attached to a *different* user → raise `OAuthError("This <provider> account is linked to another user")`, redirected to the SPA as `/auth/complete#error=link_collision&message=…`. The user's original session is untouched.
- **Anonymous "different-provider-different-account" still produces separate Users.** The login flow (`resolve_*`) is unchanged — signing out and signing back in via a fresh provider account still creates a brand-new `User` row, exactly per §6's identity policy. Linking only happens when the user is *already authenticated*.
